...
- `本项目联网口径`：使用 Brave Search 作为外部公开网页检索源，再把检索结果作为上下文交给 DeepSeek 生成回答；这应标注为 `DeepSeek + Brave 外部检索增强`，不是 DeepSeek 原生联网搜索。
...
本系统当前实现选择第三种口径：`DeepSeek + Brave Search`。采样时 Brave 返回的网页结果会写入 `citations_json`，DeepSeek 的回答 prompt 会明确要求“只基于 Brave Search 结果回答”，并禁止自称 DeepSeek 自己联网。
...
- `DeepSeek`：标准公开 API 文档没有像其他家那样明确列出通用联网搜索请求参数；但官方又明确写了 Claude Code 集成支持 Web Search。本项目为了客户侧六平台联网口径，采用 `DeepSeek + Brave Search 外部检索增强`，必须和 DeepSeek 原生联网搜索区分。
